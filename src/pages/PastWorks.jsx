...
import { getPastPaintings, getPage } from '../lib/content'
...
export default function PastWorks() {
  ...
  const p = getPage('past-works') || {}

  return (
    <div className="min-h-screen bg-brand-bg text-brand-ink" {...(p._id ? { 'data-sb-object-id': p._id } : {})}>
      <SEOHead
        title={p.seo_title || 'Past Works'}
        description={p.seo_description || "An archive of Sadaf's works now in private collections."}
      />
      ...
      <section className="border-b border-gray-100 bg-brand-bg pt-28 pb-16 sm:px-6 md:pt-32 md:pb-20">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="max-w-2xl text-sm font-light leading-relaxed text-brand-muted sm:text-[15px] md:text-base" data-sb-field-path="hero_description">
            {p.hero_description || 'An archive of original paintings now in private collections. To inquire about commissioning a similar piece, please get in touch.'}
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          {paintings.length === 0 ? (
            <p className="text-brand-muted text-sm font-light" data-sb-field-path="empty_state">
              {p.empty_state || 'No archived works yet.'}
            </p>
          ) : (
                ...
                >
                  <Link to={`/paintings/${painting.slug}`} className="relative aspect-[4/5] overflow-hidden bg-transparent">
                    ...
                    <span className="absolute top-3 left-3 bg-white/90 px-2.5 py-1 text-[9px] uppercase tracking-[0.18em] text-brand-muted">
                      {p.tile_status_label || 'In Private Collection'}
                    </span>
