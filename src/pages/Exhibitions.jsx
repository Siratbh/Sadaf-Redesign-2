        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16 items-center">
          ...
          >
            <div className="relative w-full aspect-[4/3] md:aspect-[5/4] overflow-hidden bg-gray-100">
              ...
              {ex.exhibition_type && (
                <span className="absolute top-4 left-4 bg-brand-bg/95 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.22em] text-brand-ink">
                  {ex.exhibition_type}
                </span>
              )}
            </div>
          ...
          >
            ...
            <h2 className="font-serif italic text-brand-ink leading-[1.02] text-4xl sm:text-5xl md:text-6xl lg:text-[64px]">
              {ex.title}
            </h2>
            <div className="mt-6 md:mt-8 space-y-2">
              <MetaLine ex={ex} />
              {ex.description && (
                <p className="pt-4 max-w-md text-[14px] md:text-[15px] leading-[1.7] font-light text-brand-muted">
                  {ex.description}
                </p>
              )}
            </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-12 items-end">
            <div className="md:col-span-3 lg:col-span-4">
              ...
              <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.28em] text-brand-ink">
                {ex.exhibition_type || 'Exhibition'}
              </p>
            </div>
            <div className="md:col-span-9 lg:col-span-8">
              <h2 className="font-serif italic text-brand-ink leading-[0.95] uppercase text-5xl sm:text-6xl md:text-7xl lg:text-[96px] xl:text-[112px] tracking-tight">
                {ex.title}
              </h2>
    ...
    <div className="group flex flex-col">
      <Wrapper {...wrapperProps} className="block">
        <div className="relative w-full aspect-[4/5] overflow-hidden bg-gray-100">
          ...
          {ex.exhibition_type && (
            <span className="absolute top-3 left-3 bg-brand-bg/95 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.22em] text-brand-ink">
              {ex.exhibition_type}
            </span>
          )}
          {linked && (
        ...
        <div className="mt-4 md:mt-5">
          <h3 className="font-serif italic text-brand-ink text-xl md:text-2xl leading-[1.15] group-hover:text-brand-accent transition-colors">
            {ex.title}
          </h3>
    ...
    <div className="mb-20 md:mb-28 last:mb-0">
      <div className="flex items-baseline gap-6 md:gap-10 mb-8 md:mb-12">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-[0.28em] text-brand-muted">
          {items.length} {items.length === 1 ? 'exhibition' : 'exhibitions'}
        </span>
      </div>
    ...
    <div className="min-h-screen bg-brand-bg text-brand-ink">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 pb-12 md:pt-32 md:pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          >
            The <i className="font-serif italic">Archive</i>
          </Motion.h1>
          <Motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
      ...
      {/* Hero featured */}
      {featured[0] && <HeroFeatured ex={featured[0]} />}

      {/* NEXT featured */}
      {featured[1] && <NextFeatured ex={featured[1]} />}

      {/* Archive grid */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-20 md:pt-28 pb-24 md:pb-32">
        <div className="mb-12 md:mb-16">
          <p className="text-[10px] font-semibold uppercase tracking-[0.32em] text-brand-muted mb-3">
            Exhibition Archive
          </p>
          <p className="font-serif italic text-brand-ink text-2xl md:text-3xl lg:text-4xl leading-[1.15] max-w-2xl">
            Every show, residency, and fair — from the earliest to the most recent.
          </p>
        </div>
        ...
        {years.length === 0 ? (
          ...
          years.map((year) => (
            <YearSection key={year} year={year} items={byYear[year]} />
          ))
        )}
      </section>
    </div>
